<?php 
	class Admin_model extends CI_model{
		...
		public function check_address_default($user_id){
			$isDefault =  $this->db->where('user_id', $user_id)
					->where('is_default', 1)
					->get('address');
			if($isDefault->num_rows() > 0){
				true;
			}else{
				false;
			}
		...
		public function getUserByIdForedit($id){
			return $this->db->where('id', $id)
					->get('users')->result_array();
		}

		public function doUpdateUser($data, $id){
			$this->db->where('id', $id)
						->update('users', $data);
